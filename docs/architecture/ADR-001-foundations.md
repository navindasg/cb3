...
| D10 | **SUPERSEDED (Act 0 rework, 2026-06).** Originally: *map is a bottom-to-top `Stratum` registry; zones declare position relative to a named stratum anchor.* Replaced by a **2D flag-revealed `OverworldDef`** (`engine/types/overworld.ts` + `content/overworld.ts`): regions at world-cell `(x,y)`, each gated by a reveal flag, panned in 2D. Still grows upward (beanstalk → sky → space) so the genre-reveal holds; the `StratumDef`/`ZoneDef`/`render/Map`/`render/mapModel` stack was deleted. See DESIGN §3 implementation note. | Brief 1: CB2's fixed bitmap with hardcoded offsets cannot grow. The 2D overworld keeps the upward growth while letting Act 0's ground spread horizontally. |
...
**SUPERSEDED (Act 0 rework, 2026-06) — see D10 + DESIGN §3.** The realized map is a 2D flag-revealed `OverworldDef` (`content/overworld.ts`), panned by `render/Overworld.ts`, not a virtualized bottom-to-top stratum scroll. It still grows upward (the beanstalk reveal extends the revealed bounds toward the sky). The notes below describe the original, retired plan.
...
`aria-hidden` on decorative ASCII; `role="img"` + `aria-label` on meaningful scenes/map (with a text description of player location). `prefers-reduced-motion: reduce` ⇒ `animation:none` on all glow/comet/sun motion (static glow color stays). Viewport `width=device-width, initial-scale=1`; never disable zoom. `rem`/`em` sizing. Horizontal-scroll container for wide art on narrow screens. Act 4 audio gated behind the descent click that also calls `audioContext.resume()`. **SUPERSEDED (Act 0 rework, 2026-06):** the original "visible focusable `<input>` for a typed-secret box (mobile keyboard)" decision was reversed — CB3 secrets are **interaction-based** (feed exactly N / throw at / hold exactly N), not typed, so no secret `<input>` ships (see DESIGN §8). The cold-start e2e asserts there is no stray input.
